...
export default function DocDash() {
    ...
    fetch('http://localhost:5000/signups')
      ...
      .then((json) => {
        ...
        if (json && json.ok && Array.isArray(json.data)) {
          const patients = json.data.filter((s: Signup) => (s.userType || '').toLowerCase() === 'patient');
          setData(patients);
        } else {
  ...
  const getImageUrl = (p?: string) => {
    ...
    // backend serves uploads at localhost:5000/uploads
  ...
  color="black"
    ...
    >
      <Flex maxW="1100px" mx="auto" gap={6} px={4} direction={{ base: 'column', md: 'row' }}>
        <Box flex={1} p={6}>
          <Box mb={4}>
            <Heading size="lg">Patient Signups</Heading>
            <Text fontSize="sm" color="gray.700">Showing signups filtered for patient.</Text>
          </Box>
      ...
      <Box mb={4} p={1} borderRadius="6px" bg="rgba(255,255,255,0.8)">
        ...
        <Text fontSize="sm" color="black"><strong>Saved at:</strong> {latest.savedAt ? (() => { try { return new Date(latest.savedAt).toLocaleString(); } catch { return latest.savedAt; } })() : '-'}</Text>
          ...
          {data.length === 0 && (
                    <tr>
            <td colSpan={5} style={{ padding: 16 }}><Text>No patient signups found</Text></td>
                    </tr>
                  )}
            {data.map((d, i) => (
